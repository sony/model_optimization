...
def compute_kpi_data(in_model: Any,
    ...
    min_cfg = graph.get_min_candidates_config()
    max_cfg = graph.get_max_candidates_config()

    ######################################
    # Compute parameters sum
    ######################################
    min_parameters_sum = np.sum(compute_weights_size_kpi(mp_cfg=min_cfg, graph=graph, fw_info=fw_info))
    max_parameters_sum = np.sum(compute_weights_size_kpi(mp_cfg=min_cfg, graph=graph, fw_info=fw_info))


    ######################################
    # Compute max activation tensor
    ######################################
    min_precision_largest_tensor = np.max(compute_activation_output_size_kpi(mp_cfg=min_cfg, graph=graph))
    max_precision_largest_tensor = np.max(compute_activation_output_size_kpi(mp_cfg=max_cfg, graph=graph))
...
def compute_weights_size_kpi(mp_cfg: List[int], graph: Graph, fw_info: FrameworkInfo):
    weights_memory = []
...
def compute_activation_output_size_kpi(mp_cfg: List[int], graph: Graph):
    activation_memory = []
