...
from model_compression_toolkit.common.target_platform.operators import OperatorSetConcat, OperatorsSetBase



class OperationsSetToLayers(TargetPlatformModelComponent):
    """
    ...
    def __init__(self,
        ...
        _current_target_platform_capabilities.get().remove_opset_from_not_used_list(op_set_name)
...
class OperationsToLayers:
    ...
    def validate_op_sets(self):
        ...
        for ops2layers in self.op_sets_to_layers:
            ...
            # Assert that opset in the current TargetPlatformModel and has a unique name.
            is_opset_in_model = _current_target_platform_capabilities.get().target_platform_model.is_opset_in_model(ops2layers.name)
            ...
            for layer in ops2layers.layers:
                qco_by_opset_name = _current_target_platform_capabilities.get().target_platform_model.get_config_options_by_operators_set(ops2layers.name)
