...
from model_compression_toolkit.common.target_platform.targetplatform2framework.target_platform_model_component import TargetPlatformModelComponent
...
from model_compression_toolkit.common.target_platform.targetplatform2framework.current_target_platform_capabilities import _current_target_platform_capabilities
...
class TargetPlatformCapabilities(ImmutableClass):
    ...
    def __init__(self,
                 target_platform_model: TargetPlatformModel,
                 name: str = "base"):
        """

        Args:
            target_platform_model (TargetPlatformModel): Modeled hardware to attach framework information to.
            name (str): Name of the TargetPlatformCapabilities.
        """

        super().__init__()
        self.name = name
        assert isinstance(target_platform_model, TargetPlatformModel), f'Target platform model that was passed to TargetPlatformCapabilities must be of type TargetPlatformModel, but has type of {type(target_platform_model)}'
        self.target_platform_model = target_platform_model
        ...
        self.__tb_model_opsets_not_used = [s.name for s in target_platform_model.operator_set]
    ...
    def get_layers_by_opset_name(self, opset_name: str) -> List[Any]:
        ...
        """
        opset = self.target_platform_model.get_opset_by_name(opset_name)
        if opset is None:
    ...
    def get_fusing_patterns(self) -> List[List[Any]]:
        ...
        """
        res = []
        for p in self.target_platform_model.fusing_patterns:
    ...
    def get_info(self) -> Dict[str, Any]:
        ...
        return {"Target Platform Capabilities": self.name,
                "Target platform model": self.target_platform_model.get_info(),
    ...
    def append_component(self, tpc_component: TargetPlatformModelComponent):
        """
    ...
    def __enter__(self):
        ...
        """
        _current_target_platform_capabilities.set(self)
        return self
    ...
    def __exit__(self, exc_type, exc_value, tb):
        ...
        _current_target_platform_capabilities.reset()
    ...
    def get_default_op_qc(self) -> OpQuantizationConfig:
        ...
        """
        return self.target_platform_model.get_default_op_quantization_config()
    ...
    def get_qco_by_node(self,
        ...
        return self.target_platform_model.default_qco
    ...
    def _get_config_options_mapping(self) -> Tuple[Dict[Any, QuantizationConfigOptions],
        ...
        for op2layers in self.op_sets_to_layers.op_sets_to_layers:
            for l in op2layers.layers:
                qco = self.target_platform_model.get_config_options_by_operators_set(op2layers.name)
                if qco is None:
                    qco = self.target_platform_model.default_qco
    ...
    def remove_fusing_names_from_not_used_list(self):
        ...
        """
        for f in self.target_platform_model.fusing_patterns:
    ...
    def remove_opset_from_not_used_list(self,
        ...
        """
        if opset_to_remove in self.__tb_model_opsets_not_used:
            self.__tb_model_opsets_not_used.remove(opset_to_remove)
    ...
    def raise_warnings(self):
        ...
        """
        for op in self.__tb_model_opsets_not_used:
            Logger.warning(f'{op} is defined in target platform model, but is not used in target platform capabilities.')
