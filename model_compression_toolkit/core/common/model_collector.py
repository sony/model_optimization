        ...
        for td, sc in zip(tensor_data, self.stats_containers_list):
            if isinstance(sc, (list, tuple)):
                if not isinstance(td, (list, tuple)):
                    Logger.critical('"tensor_data" must be a list or a tuple if the model tensor_list is a list or a tuple') # pragma: no cover
                if len(sc) != len(td):
                    Logger.critical('"tensor_data" and the model tensor_list must be of the same length') # pragma: no cover
