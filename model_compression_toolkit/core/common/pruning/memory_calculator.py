...
class MemoryCalculator:
    ...
    def _get_exit_node_input_mask(self,
        ...
        if len(kernel_attr) != 1:
            Logger.critical(f"Expected to found a single attribute but found {len(kernel_attr)} for node {node}")
        ...
        if ic_axis is None or int(ic_axis) != ic_axis:
            Logger.critical(f"Expected input channel axis to be an integer but is {ic_axis} for node {node}")
    ...
    def get_pruned_node_num_params(self,
        ...
        for w_attr, w in node.weights.items():
            ...
            if len(io_axis) != 1:
                Logger.critical(f"Each weight should have exactly one corresponding IO axis, but is {io_axis} ")
        ...
        if output_mask is not None:
            ...
            if channel_axis is None:
                Logger.critical("Channel axis is not defined")
            ...
            if all(isinstance(sublist, list) for sublist in node.output_shape):
                ...
                if all(len(sublist) > channel_axis and sublist[channel_axis] == compare_value for sublist in node.output_shape):
                    ...
                    Logger.critical("Number of out channels are not the same for all outputs of the node")
            else:
    ...
    def _prune_tensor(self,
        ...
        if w.shape[axis] != len(mask):
            Logger.critical(f"Expected mask length {len(mask)}, found {w.shape[axis]}.")
    ...
    def get_node_nparams_with_padded_channels(self,
        ...
        if not (num_oc >= 1 and int(num_oc) == num_oc):
            Logger.critical(f"Expected number of output channels to be a non-negative integer but is {num_oc}")
