...
class FullyQuantizedLayerWrapper(torch.nn.Module):
    ...
    def _quantize_weights(self, n):
        self._set_weights_to_quantize(n)
    ...
    def _set_weights_to_quantize(self, n):
        ...
        self.float_weights = [n.get_weights_by_keys(attr) for attr in self.weight_attrs]
    ...
    def _get_quantized_weights(self):
        ...
        assert len(quantizers)==len(self.float_weights)
        for float_weight, quantizer in zip(self.float_weights, quantizers):
            # for each attribute
