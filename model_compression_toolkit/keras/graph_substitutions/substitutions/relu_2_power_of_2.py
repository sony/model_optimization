...
from model_compression_toolkit.keras.constants import KERNEL, BIAS, RELU_MAX_VALUE


class ReLU2PowerOfTwo(common.BaseSubstitution):
    """
    Substitution to scale the weights of two linear nodes, and the move the bound of non-linear between them
    (if bounded) in order to use the entire constrained range when activations are quantized.
    """

    def __init__(self):
        """
        Initialize a ReLU2PowerOfTwo object.
        """
