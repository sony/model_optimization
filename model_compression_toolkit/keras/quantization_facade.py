...
def keras_post_training_quantization_mixed_precision(in_model: Model,
    ...
    """
     ...
     statistics. Then, using a configuration file for setting bit widths, we set a bit width
     for each layer, and the model is quantized (both coefficients and activations by default).
     ...
     Args:
         ...
         network_editor (List[EditRule]): List of EditRules. Each EditRule consists of a node filter and an action to change quantization settings of the filtered nodes.
         gptq_config (GradientPTQConfig): Configuration for using GPTQ (e.g. optimizer).
         bit_widths_config_filepath (str): Path to configuration file to set bit widths for different layers.
         analyze_similarity (bool): Whether to plot similarity figures within TensorBoard (when logger is enabled) or not.
     ...
     Examples:
         ...
         >>> import model_compression_toolkit as mct
         >>> quantized_model, quantization_info = mct.keras_post_training_quantization_mixed_precision(model, repr_datagen)
