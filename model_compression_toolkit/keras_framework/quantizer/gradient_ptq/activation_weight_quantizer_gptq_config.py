...
from model_compression_toolkit.keras_framework.quantizer.gradient_ptq.weight_quantizer import TrainableWeightQuantizer
from model_compression_toolkit.keras_framework.quantizer.gradient_ptq.activation_quantizer import TrainableQuantizer
from model_compression_toolkit.keras_framework.quantizer.gradient_ptq.base_quantizer_gptq_config import BaseQuantizeConfig
from model_compression_toolkit.keras_framework.constants import KERNEL
import numpy as np


class ActivationAndWeightQuantizeConfig(BaseQuantizeConfig):
    """
    QuantizeConfig to quantize the weights and activations of a layer
    """
    ...
    def __init__(self,
        ...
        Args:
            ...
            threshold_values: Thresholds to use for weight quantization.
        ...
        self.weight_quantizer = TrainableWeightQuantizer(num_bits=weight_num_bits,
    ...
    def update_layer_quantization_params(self, layer):
        ...
        weight_quant_config = {'weights_quantization_params': self.weight_quantizer.calc_quant_config(layer)}
        activation_quant_config = {'activation_quantization_params': self.activation_quantizer.calc_quant_config(layer)}

        return weights, weight_quant_config, activation_quant_config
    ...
    def get_trainable_quantizer_parameters(self):
        ...
        """
        trainable_params = []
        trainable_params.extend(self.weight_quantizer.get_trainable_parameters())
        trainable_params.extend(self.activation_quantizer.get_trainable_parameters())
    ...
    def __eq__(self, other: Any) -> bool:
        ...
        """
        if not isinstance(other, ActivationAndWeightQuantizeConfig):
            return False
