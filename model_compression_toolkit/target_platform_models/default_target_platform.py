...
def get_default_target_platform_model() -> TargetPlatformModel:
    """
    A method that generates a default TargetPlatformModel, with base 8-bit quantization configuration and 8, 4, 2
    bits configuration list for mixed-precision quantization.
    NOTE: in order to generate a TargetPlatformModel with different configurations but with the same Operators Sets
    (for tests, experiments, etc.), use this method implementation as a test-case, i.e., override the
    'get_op_quantization_configs' method and use its output to call 'generate_target_platform_model' with your configurations.

    Returns: A TargetPlatformModel object.

    """
    base_config, mixed_precision_cfg_list = get_op_quantization_configs()
    return generate_target_platform_model(default_config=base_config,
                                          base_config=base_config,
                                          mixed_precision_cfg_list=mixed_precision_cfg_list,
                                          name='default_target_platform_model')


def get_op_quantization_configs() -> Tuple[OpQuantizationConfig, List[OpQuantizationConfig]]:
    """
    Creates a default configuration object for 8-bit quantization, to be used to set a default TargetPlatform model.
...
def generate_target_platform_model(default_config: OpQuantizationConfig,
    ...
    Args
        default_config: A default OpQuantizationConfig to set as the TargetPlatformModel default configuration.
        ...
        mixed_precision_cfg_list: A list of OpQuantizationConfig to be used as the TargetPlatformModel mixed-precision
    ...
    with generated_tp_model:
        ...
        tp.OperatorsSet("ConvTranspose", mixed_precision_configuration_options)
