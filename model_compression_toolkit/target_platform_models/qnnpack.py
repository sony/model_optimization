...
def get_qnnpack_model():
    ...
    eight_bits = tpc.OpQuantizationConfig(
        activation_quantization_method=tpc.QuantizationMethod.UNIFORM,
        weights_quantization_method=tpc.QuantizationMethod.SYMMETRIC,
        activation_n_bits=8,
        weights_n_bits=8,
    ...
    qnnpack_model = tpc.TargetPlatformModel(default_configuration_options, name='qnnpack')
    ...
    with qnnpack_model:
        ...
        batchnorm = tpc.OperatorsSet("BatchNorm")
        relu = tpc.OperatorsSet("Relu")
        linear = tpc.OperatorsSet("Linear")

        tpc.Fusing([conv, batchnorm, relu])
        tpc.Fusing([conv, batchnorm])
        tpc.Fusing([conv, relu])
        tpc.Fusing([linear, relu])
