...
def get_tflite_target_platform_model():

    # Create a quantization config. A quantization configuration defines how an operator
    # should be quantized on the modeled target platform. In TFLite
    ...
    with tflite_model:
        ...
        squeeze = tpc.OperatorsSet("Squeeze",
                                   qc_options=tpc.get_default_quantization_config_options().clone_and_edit(
                                            quantization_preserving=True))
