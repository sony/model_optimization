...
class QuantizationAwareTrainingTest(BaseKerasFeatureNetworkTest):
    ...
    def run_test(self, **kwargs):
        ...
        if self.finalize:
            ptq_model = mct.qat.keras_quantization_aware_training_finalize_experimental(ptq_model)
...
class QATWrappersTest(BaseKerasFeatureNetworkTest):
    ...
    def run_test(self, **kwargs):
        ...
        if self.finalize:
            # QAT finalize model
            qat_finalize_model = mct.qat.keras_quantization_aware_training_finalize_experimental(qat_model)
...
class QATWrappersMixedPrecisionCfgTest(MixedPrecisionActivationBaseTest):
    ...
    def run_test(self, **kwargs):
        ...
        config = mct.core.CoreConfig(
            quantization_config=QuantizationConfig(custom_tpc_opset_to_layer={"Input": ([layers.InputLayer],)})
        )
