...
class KmeansQuantizerTestBase(BaseKerasFeatureNetworkTest):
    ...
    def get_target_platform_capabilities(self):
        tp_model = generate_test_tp_model({'weights_quantization_method': self.quantization_method,
                                      'weights_n_bits': self.weights_n_bits,
                                      'activation_n_bits': 4})
        return generate_tpc_for_keras(name="kmean_quantizer_test", tp_model=tp_model)
