...
TWO_BIT_QUANTIZATION = generate_tpc_for_keras(name="two_bit_network_test",
                                              tp_model=generate_test_tp_model({'weights_n_bits': 2,
                                                                                       'activation_n_bits': 2}))

EIGHT_BIT_QUANTIZATION = generate_tpc_for_keras(name="eight_bit_network_test",
                                                tp_model=generate_test_tp_model({'weights_n_bits': 8,
                                                                                         'activation_n_bits': 8}))

FLOAT_QUANTIZATION = get_16_bits_target_platform_capabilities_model("float_network_test")
...
def run_mode(target_platform_capabilities):
    if target_platform_capabilities is FLOAT_QUANTIZATION:
        return RunMode.FLOAT
    elif target_platform_capabilities is EIGHT_BIT_QUANTIZATION:
...
class NetworkTest(object):
    ...
    def compare(self, inputs_list, quantized_model, qc, target_platform_capabilities):
        ...
        if run_mode(target_platform_capabilities) == RunMode.FLOAT:
            self.unit_test.assertTrue(np.isclose(cs, 1, 0.001), msg=f'fail cosine similarity check: {cs}')
        elif run_mode(target_platform_capabilities) == RunMode.EIGHT:
        ...
        elif run_mode(target_platform_capabilities) == RunMode.TWO:
            self.unit_test.assertTrue(np.isclose(cs, 0, atol=0.5), msg=f'fail cosine similarity check:{cs}')
        if run_mode(target_platform_capabilities) == RunMode.EIGHT:
    ...
    def run_network(self, inputs_list, qc, target_platform_capabilities):
        ...
        if self.gptq:
            ...
            ptq_model, quantization_info = mct.keras_post_training_quantization(self.model_float,
                                                                                ...
                                                                                n_iter=self.num_calibration_iter,
                                                                                tpc=target_platform_capabilities)
        self.compare(inputs_list, ptq_model, qc, target_platform_capabilities)
