...
def get_base_config():
    return tpc.OpQuantizationConfig(
...
def prepare_graph(in_model, base_config, bitwidth_candidates):
    tp_model = generate_tp_model_with_activation_mp(base_config, bitwidth_candidates)
    target_platform_capabilities = generate_activation_mp_tpc_keras(name="candidates_filter_test", tp_model=tp_model)
