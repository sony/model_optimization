...
def prep_test(model, mp_bitwidth_candidates_list, random_datagen):
    ...
    kpi_data_dict = mct.keras_kpi_data(in_model=model,
...
class TestKPIData(unittest.TestCase):
    ...
    def verify_results(self, kpi, sum_parameters, max_tensor, expected_bitwidth):
        ...
        self.assertTrue(kpi.weights_memory == sum_parameters * expected_size_factor,
                        f"Expects weights_memory to be {sum_parameters * expected_size_factor} "
                        f"but result is {kpi.weights_memory}")
        self.assertTrue(kpi.activation_memory == max_tensor * expected_size_factor,
