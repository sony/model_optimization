...
class TestKerasQuantConfigFiltering(unittest.TestCase):
    ...
    def test_config_filtering(self):
        node = FunctionalNode('node',{FUNCTION: TFOpLambda(tf.multiply).symbol},
                              [1, 8], [1, 8], {}, TFOpLambda,
                              [], {}, functional_op=tf.multiply)
        next_node1 = FunctionalNode('next_node',{FUNCTION: TFOpLambda(tf.add).symbol},
                              [1, 8], [1, 8], {}, TFOpLambda,
                              [], {}, functional_op=tf.add)
        next_node2 = FunctionalNode('next_node',{FUNCTION: TFOpLambda(tf.divide).symbol},
                              [1, 8], [1, 8], {}, TFOpLambda,
