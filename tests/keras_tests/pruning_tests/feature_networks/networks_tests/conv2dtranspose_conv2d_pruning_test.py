...
class Conv2DTransposetoConv2DPruningTest(PruningKerasFeatureTest):
    ...
    def get_pruning_config(self):
        add_const_importance_metric(first_num_oc=5, second_num_oc=4)
        return mct.pruning.PruningConfig(importance_metric=ConstImportanceMetric.CONST)

    def create_networks(self):
        inputs = layers.Input(shape=self.get_input_shapes()[0][1:])
        x = layers.Conv2DTranspose(filters=5, kernel_size=1)(inputs)
        if self.use_bn:
    ...
    def compare(self, quantized_model, float_model, input_x=None, quantization_info=None):
        ...
        self.unit_test.assertTrue(prunable_convtrans_layers[0].filters == 4)
        self.unit_test.assertTrue(np.all(prunable_convtrans_layers[0].kernel.numpy() == dense_convtrans_layers[0].kernel.numpy()[:, :, :-1, :]))
        self.unit_test.assertTrue(np.all(prunable_convtrans_layers[0].bias.numpy() == dense_convtrans_layers[0].bias.numpy()[:-1]))
