...
from tests.keras_tests.pruning_tests.feature_networks.constant_importance_metric import add_const_importance_metric, \
...
class Conv2DTransposetoConv2DPruningTest(PruningKerasFeatureTest):
    ...
    def create_networks(self):
        ...
        x = layers.Conv2DTranspose(filters=6, kernel_size=1)(inputs)
        if self.use_bn:
    ...
    def compare(self, quantized_model, float_model, input_x=None, quantization_info=None):
        ...
        is_first_layer_pruned = prunable_convtrans_layers[0].filters == 6 - self.simd
        is_second_layer_pruned = prunable_conv_layers[0].filters == 4 - self.simd
        ...
        if self.use_constant_importance_metric:
            ...
            self.unit_test.assertTrue(np.all(prunable_convtrans_layers[0].bias.numpy() == dense_convtrans_layers[0].bias.numpy()[:-self.simd]))
            ...
            self.unit_test.assertTrue(np.all(prunable_conv_layers[0].bias.numpy() == dense_conv_layers[0].bias.numpy()))

        if is_first_layer_pruned:
            self.unit_test.assertTrue(np.all(prunable_convtrans_layers[1].kernel.numpy() == dense_convtrans_layers[1].kernel.numpy()))
            self.unit_test.assertTrue(np.all(prunable_convtrans_layers[1].bias.numpy() == dense_convtrans_layers[1].bias.numpy()))
