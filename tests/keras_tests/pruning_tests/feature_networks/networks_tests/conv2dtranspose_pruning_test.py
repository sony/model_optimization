...
class Conv2DTransposePruningTest(PruningKerasFeatureTest):
    ...
    def compare(self, quantized_model, float_model, input_x=None, quantization_info=None):
        dense_layers = get_layers_from_model_by_type(float_model, layers.Conv2DTranspose)
        prunable_layers = get_layers_from_model_by_type(quantized_model, layers.Conv2DTranspose)

        is_first_layer_pruned = prunable_layers[0].filters == 6 - self.simd
        is_second_layer_pruned = prunable_layers[1].filters == 4 - self.simd
