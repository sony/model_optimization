...
def get_16_bits_target_platform_capabilities_model(name):
    tp_model = generate_test_tp_model({'weights_n_bits': 16,
                                       'activation_n_bits': 16})
    return generate_tpc_for_keras(name=name, tp_model=tp_model)


def get_quantization_disabled_keras_tp_model(name):
    tp_model = generate_test_tp_model({'enable_weights_quantization': False,
                                  'enable_activation_quantization': False})
    return generate_tpc_for_keras(name=name, tp_model=tp_model)


def generate_activation_mp_tpc_keras(tp_model, name="activation_mp_keras_tp_model"):

    tpc_keras = tp.TargetPlatformCapabilities(tp_model,
                                              name=name)
    with tpc_keras:
