...
from tests.common_tests.test_target_platform_model import TEST_QCO, TEST_QC
...
class TestKerasTPModel(unittest.TestCase):
    ...
    def test_get_layers_by_op(self):
        tp_model = tp.TargetPlatformModel(tp.QuantizationConfigOptions([TEST_QC]))
        with tp_model:
            op_obj = tp.OperatorsSet('opsetA')
        tpc = TargetPlatformCapabilities(tp_model)
        with tpc:
            opset_layers = [Conv2D, LayerFilterParams(ReLU, max_value=2)]
            tp.OperationsSetToLayers('opsetA', opset_layers)
        self.assertEqual(tpc.get_layers_by_opset_name('opsetA'), opset_layers)
        self.assertEqual(tpc.get_layers_by_opset(op_obj), opset_layers)
    ...
    def test_get_layers_by_opconcat(self):
        ...
        self.assertEqual(tpc.get_layers_by_opset_name('opsetA_opsetB'), opset_layers_a + opset_layers_b)
        self.assertEqual(tpc.get_layers_by_opset(op_concat), opset_layers_a + opset_layers_b)
    ...
    def test_qco_by_keras_layer(self):
        ...
        tp_model = tp.TargetPlatformModel(default_qco, name='test')
        with tp_model:
            mixed_precision_configuration_options = tp.QuantizationConfigOptions([TEST_QC,
                                                                                  TEST_QC.clone_and_edit(weights_n_bits=4),
                                                                                  TEST_QC.clone_and_edit(weights_n_bits=2)],
                                                                                 base_config=TEST_QC)
        ...
        tpc = tp.TargetPlatformCapabilities(tp_model, name='fw_test')
        with tpc:
        ...
        conv_qco = tpc.get_qco_by_node(conv_node)
        tanh_qco = tpc.get_qco_by_node(tanh_node)
        relu_qco = tpc.get_qco_by_node(relu_node)
    ...
    def test_opset_not_in_tp_model(self):
        default_qco = tp.QuantizationConfigOptions([TEST_QC])
        tp_model = tp.TargetPlatformModel(default_qco)
    ...
    def test_keras_fusing_patterns(self):
        ...
        fusings = tpc.get_fusing_patterns()
...
class TestGetKerasTPModelAPI(unittest.TestCase):

    def test_get_keras_tp_models(self):
    ...
    def test_get_keras_not_supported_model(self):
        ...
        self.assertEqual('TargetPlatformModel named should_not_support is not supported for framework tensorflow', str(e.exception))
