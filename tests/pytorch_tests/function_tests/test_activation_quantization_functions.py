...
class TestActivationQuantizationFunctions(unittest.TestCase):
    ...
    def test_symmetric_quantization(self):
        for signed in [True, False]:
            ...
            float_tensor = torch.tensor([-2.1, -1.1, 0, 1, 1.1])
            if signed:
                expected_quantized_tensor = torch.tensor([-2, -1, 0, 1, 1])
            else:
                expected_quantized_tensor = torch.tensor([0, 0, 0, 1, 1])
    ...
    def test_uniform_quantization(self):
        activation_n_bits = 2
        float_tensor = torch.tensor([-2.1, -1.1, 0.0, 1.0, 1.1])
        params_dict = {'min_negative_max_positive': {RANGE_MIN: -2,
                       ...
                       'min_positive': {RANGE_MIN: 1,
                                        RANGE_MAX: 3,
                                        'expected_quantized_tensor': torch.tensor([0, 0, 0, 1, 1])},
                       'max_negative': {RANGE_MIN: -3,
                                        RANGE_MAX: -1,
                                        'expected_quantized_tensor': torch.tensor([-2, -1, 0, 0, 0])}
                       }
