...
class Conv2dModelForPrunning(torch.nn.Module):

    def __init__(self, use_bn, activation=None):
        super(Conv2dModelForPrunning, self).__init__()
        self.conv1 = torch.nn.Conv2d(in_channels=3, out_channels=8, kernel_size=1)
        self.bn = torch.nn.BatchNorm2d(8)
        self.conv2 = torch.nn.Conv2d(in_channels=8, out_channels=6, kernel_size=2)
        self.conv3 = torch.nn.Conv2d(in_channels=6, out_channels=2, kernel_size=3)
...
class Conv2DPruningTest(PruningPytorchFeatureTest):
    """
    ...
    def create_networks(self):
        return Conv2dModelForPrunning(use_bn=self.use_bn, activation=self.activation_layer)

    def compare(self, quantized_model, float_model, input_x=None, quantization_info=None):
        dense_layers = get_layers_from_model_by_type(float_model, torch.nn.Conv2d)
        prunable_layers = get_layers_from_model_by_type(quantized_model, torch.nn.Conv2d)
