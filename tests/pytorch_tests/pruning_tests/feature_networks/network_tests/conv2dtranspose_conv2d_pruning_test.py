...
class Conv2dTransposeConv2dModelForPrunning(torch.nn.Module):

    def __init__(self, use_bn, activation=None):
        super(Conv2dTransposeConv2dModelForPrunning, self).__init__()
        self.conv2dtrans1 = torch.nn.ConvTranspose2d(in_channels=3, out_channels=6, kernel_size=1)
        self.bn = torch.nn.BatchNorm2d(6)
        self.conv2 = torch.nn.Conv2d(in_channels=6, out_channels=4, kernel_size=1)
...
class Conv2dTransposeConv2dPruningTest(PruningPytorchFeatureTest):
    """
    ...
    def create_networks(self):
        return Conv2dTransposeConv2dModelForPrunning(use_bn=self.use_bn, activation=self.activation_layer)
    ...
    def compare(self, quantized_model, float_model, input_x=None, quantization_info=None):
        ...
        is_first_layer_pruned = prunable_convtrans_layers[0].out_channels == dense_convtrans_layers[0].out_channels - self.simd
        is_second_layer_pruned = prunable_conv_layers[0].out_channels == dense_conv_layers[0].out_channels - self.simd
        ...
        if self.use_constant_importance_metric:
            ...
            self.unit_test.assertTrue(np.all(torch_tensor_to_numpy(prunable_convtrans_layers[0].weight)==torch_tensor_to_numpy(dense_convtrans_layers[0].weight)[:, :-self.simd,:,:]))
            self.unit_test.assertTrue(np.all(torch_tensor_to_numpy(prunable_convtrans_layers[0].bias)==torch_tensor_to_numpy(dense_convtrans_layers[0].bias)[:-self.simd]))

            # Make sure the only in channel removed is the last channel of the second conv layer
            self.unit_test.assertTrue(np.all(torch_tensor_to_numpy(prunable_conv_layers[0].weight) == torch_tensor_to_numpy(dense_conv_layers[0].weight)[:, :-self.simd, :, :]))
            self.unit_test.assertTrue(np.all(torch_tensor_to_numpy(prunable_conv_layers[0].bias)==torch_tensor_to_numpy(dense_conv_layers[0].bias)))

        if is_first_layer_pruned:
            self.unit_test.assertTrue(np.all(torch_tensor_to_numpy(prunable_convtrans_layers[1].weight) == torch_tensor_to_numpy(dense_convtrans_layers[1].weight)))
            self.unit_test.assertTrue(np.all(torch_tensor_to_numpy(prunable_convtrans_layers[1].bias) == torch_tensor_to_numpy(dense_convtrans_layers[1].bias)))
