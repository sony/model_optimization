...
from model_compression_toolkit.quantizers_infrastructure.inferable_infrastructure.keras.quantizers.activation_inferable_quantizers.activation_lut_pot_inferable_quantizer import \
...
class BaseKerasActivationLUTQuantizerTest(BaseInferableQuantizerTest):
    ...
    def test_lut_pot_signed_inferable_quantizer(self):
        ...
        self.unit_test.assertTrue(np.all(centers / (2 ** (multiplier_n_bits - 1)) * thresholds == quantized_tensor),
    ...
    def test_lut_pot_unsigned_inferable_quantizer(self):
        ...
        self.unit_test.assertTrue(np.all(centers / (2 ** multiplier_n_bits) * thresholds == quantized_tensor),
                        "Quantized tensor values weren't assigned correctly")
...
class TestKerasActivationPOTLUTQuantizerAssertions(BaseKerasActivationLUTQuantizerTest):
    ...
    def run_test(self):
        ...
        self.test_illegal_cluster_centers_range_inferable_quantizer()
...
class TestKerasActivationPOTLUTQuantizer(BaseKerasActivationLUTQuantizerTest):
