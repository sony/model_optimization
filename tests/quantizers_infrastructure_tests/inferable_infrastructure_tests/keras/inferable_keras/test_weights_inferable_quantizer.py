...
class TestKerasWeightsSymmetricInferableQuantizerRaise(BaseInferableQuantizerTest):

    def run_test(self):
        # test_ illegal_num_of_thresholds_symmetric_inferable_quantizer
        with self.unit_test.assertRaises(Exception) as e:
            WeightsSymmetricInferableQuantizer(num_bits=8,
                                               ...
                                               channel_axis=None,
                                               input_rank=4)
        self.unit_test.assertEqual('In per-tensor quantization min/max should be of length 1 but is 2', str(e.exception))

        # test_illegal_threshold_type_symmetric_inferable_quantizer
        with self.unit_test.assertRaises(Exception) as e:
            WeightsSymmetricInferableQuantizer(num_bits=8,
                                               per_channel=True,
        ...
        # test_missing_channel_axis_symmetric_inferable_quantizer
        with self.unit_test.assertRaises(Exception) as e:
            WeightsSymmetricInferableQuantizer(num_bits=8,
                                               per_channel=True,
                                               threshold=[3., 2.],
        ...
        # test_missing_input_rank_pot_inferable_quantizer
        with self.unit_test.assertRaises(Exception) as e:
            WeightsSymmetricInferableQuantizer(num_bits=8,
                                               per_channel=True,
                                               threshold=[3., 2.],
...
class TestKerasWeightsSymmetricInferableSignedPerTensorQuantizer(BaseInferableQuantizerTest):
    ...
    def run_test(self):
        ...
        self.unit_test.assertTrue(quantizer_config['per_channel'] is False)
        self.unit_test.assertTrue(quantizer_config['channel_axis'] is None)
...
class TestKerasWeightsSymmetricInferableSignedPerChannelQuantizer(BaseInferableQuantizerTest):
    ...
    def run_test(self):
        ...
        quantizer_config = quantizer.get_config()
        self.unit_test.assertTrue(quantizer_config['num_bits'] == num_bits)
        self.unit_test.assertTrue(np.all(quantizer_config['threshold'] == thresholds))
        self.unit_test.assertTrue(quantizer_config['per_channel'] is True)
        self.unit_test.assertTrue(quantizer_config['channel_axis'] == 3)
        ...
        for i in range(len(thresholds)):
            ...
            # Assert some values are negative (signed quantization)
            self.unit_test.assertTrue(np.any(channel_slice_i < 0),
        ...
        self.unit_test.assertTrue(np.all(manually_quantized_tensor == quantized_tensor.numpy()))


class TestKerasWeightsPOTInferableQuantizerRaise(BaseInferableQuantizerTest):

    def run_test(self):
        # illegal_pot_inferable_quantizer
        with self.unit_test.assertRaises(Exception) as e:
        ...
        # test_illegal_num_of_thresholds_pot_inferable_quantizer
        with self.unit_test.assertRaises(Exception) as e:
            WeightsPOTInferableQuantizer(num_bits=8,
                                         per_channel=False,
                                         threshold=[3.0, 2.0],
        ...
        # test_illegal_threshold_type_pot_inferable_quantizer
        with self.unit_test.assertRaises(Exception) as e:
            WeightsPOTInferableQuantizer(num_bits=8,
                                         per_channel=True,
        ...
        # test_missing_channel_axis_pot_inferable_quantizer
        with self.unit_test.assertRaises(Exception) as e:
            WeightsPOTInferableQuantizer(num_bits=8,
                                         per_channel=True,
                                         threshold=[3., 2.],
...
class TestKerasWeightsPOTInferableSignedPerChannelQuantizer(BaseInferableQuantizerTest):
    ...
    def run_test(self):
        ...
        self.unit_test.assertTrue(np.all(quantizer_config['threshold'] == thresholds))
        self.unit_test.assertTrue(quantizer_config['per_channel'] is True)
        self.unit_test.assertTrue(quantizer_config['channel_axis'] == -1)
...
class TestKerasWeightsPOTInferableSignedPerTensorQuantizer(BaseInferableQuantizerTest):
...
class TestKerasWeightsUniformInferableQuantizerRaise(BaseInferableQuantizerTest):

    def run_test(self):
        # test_illegal_num_of_minmax_uniform_inferable_quantizer
        with self.unit_test.assertRaises(Exception) as e:
        ...
        with self.unit_test.assertRaises(Exception) as e:
            WeightsUniformInferableQuantizer(num_bits=8,
                                             ...
                                             channel_axis=None,
                                             input_rank=4)
        self.unit_test.assertEqual('Expected max_range to be of type list but is <class \'int\'>', str(e.exception))
...
class TestKerasWeightsUniformInferableSignedPerChannelQuantizer(BaseInferableQuantizerTest):
    ...
    def run_test(self):
        ...
        quantizer_config = quantizer.get_config()
        self.unit_test.assertTrue(quantizer_config['num_bits'] == num_bits)
        self.unit_test.assertTrue(np.all(quantizer_config['max_range'] == max_range))
        self.unit_test.assertTrue(np.all(quantizer_config['min_range'] == min_range))
        self.unit_test.assertTrue(quantizer_config['per_channel'] is True)
...
class TestKerasWeightsUniformInferableSignedPerTensorQuantizer(BaseInferableQuantizerTest):
...
class TestKerasWeightsUniformInferableZeroNotInRange(BaseInferableQuantizerTest):
