...
class TestXQuantReport(unittest.TestCase):
    ...
    def setUp(self):
        self.float_model = get_model_to_test2()
    ...
    def test_disable_quantization(self):
        return # TODO: test when implemented
        disable_act_action = mct.core.network_editor.ChangeFinalActivationQuantConfigAttr(enable_activation_quantization=False)
        disable_weight_action = mct.core.network_editor.ChangeFinalWeightsQuantConfigAttr(
            attr_name='kernel',
            enable_weights_quantization=False)
        self.xquant_config.edit_rules = [
            mct.core.network_editor.EditRule(
                filter=mct.core.network_editor.NodeTypeFilter(keras.layers.InputLayer),
                action=disable_act_action),
            mct.core.network_editor.EditRule(
                filter=mct.core.network_editor.NodeTypeFilter(keras.layers.Activation),
                action=disable_act_action),
            mct.core.network_editor.EditRule(
                filter=mct.core.network_editor.NodeTypeFilter(keras.layers.Conv2D),
                action=disable_weight_action),
            ]
