...
def get_sensitivity_evaluator(custom_metric_fn):
    ...
    with patch.object(SensitivityEvaluation, '_build_mp_model', return_value=(Mock(), Mock())):
        sensitivity_eval = SensitivityEvaluation(graph=mock_graph, mp_config=mp_cfg,
                                                 representative_data_gen=representative_data_gen, fw_info=mock_fw_info,
...
class TestMPCustomMetricFunction:
    ...
    def test_valid_metric_function(self, metric_fn, expected):
        ...
        assert isinstance(sensitivity_eval.metric_calculator, CustomMetricCalculator)
    ...
    def test_type_invalid_metric_function(self, metric_fn, expected):
        ...
        with pytest.raises(TypeError, match=f'The custom_metric_fn is expected to return float or numpy float, got {expected}'):
            sensitivity_eval.compute_metric(Mock(), Mock())
