...
def test_get_mac(minimal_tpc):
    ...
    assert fw_impl.get_node_mac_operations(nodes[11]) == 4*3*(80*10)
    assert fw_impl.get_node_mac_operations(nodes[12]) == 0
    assert fw_impl.get_node_mac_operations(nodes[13]) == (4*3*10)*5
